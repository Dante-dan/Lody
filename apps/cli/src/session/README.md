...
`session-manager.ts` installs the workspace-bound `gh-session-bin/<workspace-binding>`
wrapper and activates/refreshes its requester-bound broker context. It does not inject a
session-wide `GH_TOKEN`. `../lib/gh-shim-script.ts` chooses credentials at invocation time:
broker authorization, then owner credentials/active native gh login, then a requester-bound
managed token. `../lib/github-token-manager.ts` fetches/caches managed tokens;
`../lib/gh-token-env.ts` handles token cleanup, including legacy managed-token fingerprints.
...
These boundaries prevent automatic credential inheritance through Lody's launch paths. They
are not OS credential isolation: same-OS-user processes can still access native gh/keychains,
and shell modes that ignore or replace Lody's startup hooks are outside that protection.
...
I/O. PR and push identity comes from the selected GitHub credential, independently of git config.
