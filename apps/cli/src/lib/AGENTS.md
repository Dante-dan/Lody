...
Read context/message-flow.md before dispatch/sync changes. Never extend the deprecated WS/DO
control plane.
...
- Local session control preserves every intermediate response: new clients negotiate
  ...
  is sent.
- Route bridge attach/detach/revoke through `machine-runtime.ts` `runBridgeTransition`; no long
  waits inside transitions. Backfill enable/disable flips authorization generation inside the
  queued body; revoked workspaces must not retain backfill (S5).
- Renderer user/UI writes go through its own repo/Streams; CLI writes only agent data. Never
  restore v4-v6 write-intent envelopes, preview-comment mirrors, or proxy authorship
  (`specs/local-first-two-plane.md`). Dispatch uses renderer-authored `latestUserMsgId` plus
  the local Machine RPC fast path.
...
- `session-gc-manager.ts`: `evaluateMemoryPressure` has separate `evict`/`block` thresholds,
  both safety margins. Probes fail OPEN; unknown Windows commit growth stays `null`, never `0`.
  Sweeps must not force cached commit probes. Under pressure, recheck after a short delay before
  failing a turn; never act on cached samples, and bound eviction per call.
  [Per-OS signals](../../../../.agents/docs/cli-lib-memory-pressure.md).
...
- `task-automation/`: an agent counts as busy while its task is **in progress**, not
  ...
  working copy.
- `file-preview/` never activates Code Collab: no workspace watch, All Changes recompute, or
  Flock publish.
