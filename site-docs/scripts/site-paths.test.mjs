...
await test('compare docs are prerendered even when omitted from sidebar meta', () => {
  const paths = collectSitePaths(packageRoot);
  assert.ok(paths.includes('/docs/compare/kimi-vs-claude-code'));
  assert.ok(paths.includes('/zh/docs/compare/kimi-vs-claude-code'));
  assert.equal(paths.includes('/docs/kimi-vs-claude-code'), false);
  ...
  assert.equal(sidebar.includes('kimi-vs-claude-code'), false);
});
