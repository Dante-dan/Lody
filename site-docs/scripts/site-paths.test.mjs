...
await test('compare docs are prerendered even when omitted from sidebar meta', () => {
  const paths = collectSitePaths(packageRoot);
  assert.ok(paths.includes('/docs/compare/lody-vs-lore'));
  assert.ok(paths.includes('/zh/docs/compare/lody-vs-lore'));
  assert.equal(paths.includes('/docs/lody-vs-lore'), false);
  ...
  assert.equal(sidebar.includes('lody-vs-lore'), false);
});
