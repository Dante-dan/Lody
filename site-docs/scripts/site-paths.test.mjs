...
await test('compare docs are prerendered even when omitted from sidebar meta', () => {
  const paths = collectSitePaths(packageRoot);
  assert.ok(paths.includes('/docs/compare/workspace'));
  assert.ok(paths.includes('/zh/docs/compare/workspace'));
  assert.equal(paths.includes('/docs/workspace'), false);
  ...
  assert.equal(sidebar.includes('workspace'), false);
});
