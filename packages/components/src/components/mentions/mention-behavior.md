# Mention behavior and rationale
...
- `@` reaches every mention type through the two-level menu. Skills also retain
  their direct `$` menu for compatibility. `/` groups independent Shortcut and
  ACP command sources; only ACP candidates require a slash-only prompt. `#` does not open
