...
- Cmd/Ctrl+W is a native menu accelerator, not a registry command. Do not restore a
  ...
  `close-current-tab-or-window` to the focused product window. The shell
  ...
  quitting; auxiliary windows close normally, including a lone parent conversation.
  The chord is not listed or rebindable in keyboard settings.
