 ...
 *
 * Kimi and Grok have no equivalent list: their authentication requirement is
 * reported by the agent itself. Codex custom providers are detected only when
 * they use the exact shape generated by Lody's credential form.
 */
...
export const hasBuiltinEnvAuthentication = (
  ...
  if (agentType === 'claude') {
    return CLAUDE_ENV_AUTH_KEYS.some((key) => Boolean(env?.[key]?.trim()));
  }
