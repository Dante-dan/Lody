...
CLI 上执行。Agent Role、workspace MCP 选择、Session 管理和本地项目移除已成为 active；
Session 分叉旅程在 worktree commit 阶段暴露产品缺陷，因此保留为有证据且被阻塞的 backlog。

## 决定与范围

- 将 `LODY-MCP-001`、`LODY-ROLE-001` 和 `LODY-SESSION-002` 从有证据的
  backlog 提升为 active，并新增 active P1 `LODY-PROJECT-001`。
- `LODY-FORK-001` 继续留在 backlog。真实桌面运行创建 worktree 和目标 ACP runtime 后，
  第二次写入 `forkOperation` LoroMap 时出现 `Map value must be an object`；产品修复另行处理。
...
套件检查器确认 8 条 active 场景都与 registry 对应且 stable ID 唯一。三轮全新的 focused
真实桌面运行均通过 4 条新增 active 旅程和全部 37 个步骤；完整 active 回归通过全部 8 个
场景和 62 个步骤。分叉运行已到达异步 commit，并在 teardown 前保留 trace、CLI backlog、
截图和进程快照；registry 记录了根因。fixture 使用显式文件信号、进程证据、持久目录读取和
